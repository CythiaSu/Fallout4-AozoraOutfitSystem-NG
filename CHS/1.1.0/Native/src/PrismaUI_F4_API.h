...
namespace PRISMA_UI_API {
    ...
    public:
        ...
        // No-op in the shipped flat runtime: the inspector is not an in-game overlay, so
    ...
    // Per-view health state for GetViewHealth. kUnknown is only meaningful here, for "Ultralight isn't
    // running" or "never heard of that view".
    enum class ViewHealth : int {
        kUnknown = -1,         // Ultralight not active, or `view` is not a currently-known handle
        kCreating = 0,         // CreateView issued, view mounting
        kDomReady = 1,         // OnDomReady fired
        kLive = 2,             // healthy / interactive
        kLoadFailed = 3,       // page/view failed to load
    ...
    public:
        ...
        // missed liveness pings. Returns kUnknown if the framework isn't running or the handle is unknown.
        ...
        // height is <= 0, or the framework is older than ABI v6.
    ...
    public:
        ...
        //
        // MUST be called BEFORE SetViewOffscreen(view, true). The runtime fixes the background at view
        ...
        //
        // No-op if the framework is older than ABI v9.
