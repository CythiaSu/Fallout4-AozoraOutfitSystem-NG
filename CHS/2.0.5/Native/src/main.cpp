...
static void OnMenuCheckStudio(const char* js) {
    ...
    QueueGameTask([payload] {
        if (!g_menuOpen) return;
        if (!GetBoolSetting("bEnableOutfitWorkbench", false)) {
...
static bool OutfitWorkbenchEnabled() {
    return GetBoolSetting("bEnableOutfitWorkbench", false);
}
...
// ======== Native Functions ========
RE::BSFixedString OM_GetPluginVersion(std::monostate) { return "2.0.5"; }
