...
struct PreviewLightingPreset {
    // 原版灯保持原始功率，主要通过位置控制照明均匀度：前方中距离、
    // 左右对称、身体高度，避免上半身过亮而下半身衰减。
...
public:
        ...
        RE::BSTEventSource<RE::MenuOpenCloseEvent>*) override
    {
        ...
        if (event.opening && !g_menuOpen && g_closeAnimationPending.load()) {
            ...
            // closing View immediately; the delayed finalizer still destroys
            // it after the clear-frame window.
...
static bool PresentMenuView(const char* reason, bool ensureVisible = true) {
    ...
    // first physical input event. Keep the expensive paint pulse off the
    // normal Windows path, where it only adds startup work.
    if (IsRunningUnderWine()) {
        g_prisma->Invoke(g_view, "if(window.omForcePaint)window.omForcePaint();");
    }
    // This is a standalone menu opened from gameplay. Keep Prisma's FocusMenu
    // active so it owns the game cursor and routes Win32 mouse input to the
    // focused CEF view. The previous disableFocusMenu=true path left the CEF
    // host targeting the framework Dock (view 0) on Prisma 2.x.
    ...
    std::thread([view, openSerial] {
        for (const auto delay : { 60u, 220u }) {
            ...
            QueueGameTask([view, openSerial] {
                ...
                // Do not re-focus a view that already owns input. Repeating
                // Focus() during the first few frames can reset the native
                // cursor while the player is already moving the mouse.
                if (g_prisma->IsHidden(view) || !MenuViewHasFocus()) {
...
static std::string SavedItemDisplayName(const OE& item) {
    std::string display = item.name.empty() ? "未知服装" : item.name;
    if (item.weapon) {
        if (!item.mods.empty()) AppendDisplayLabels(display, { "已改造" });
        return display;
    }
    ...
    if (item.color) labels.push_back("颜色 " + std::to_string(*item.color));
    ...
    int* usageCountOut = nullptr)
{
    ...
    while (position < itemsEnd) {
        ...
        if (displayName.empty()) displayName = "未知服装";
...
static void FinalizeCloseForNativeMenu() {
    ...
    if (g_prisma && closingView != 0 && g_prisma->IsValid(closingView)) {
        ...
        LogLine("2.0 UI native menu transition destroyed closing view=" + std::to_string(closingView));
    }
...
static void OnMenuEquipSlot(const char* js) {
    ...
    QueueGameTask([payload] {
        ...
        const int slot = std::clamp(json::getInt(payload, "slot", g_curSlot), 1, MAX_SLOTS);
        auto* actor = ResolveMenuActorByFormID(json::getHexID(payload, "targetId", 0));
        if (!actor) return SendUiResult("equip", false, slot, "目标不可用");
        SetMenuActor(actor);
...
static void OnMenuCheckStudio(const char* js) {
    ...
    QueueGameTask([payload] {
        ...
        if (!GetBoolSetting("bEnableOutfitWorkbench", true)) {
            SendUiResult("studioCheck", false, g_curSlot, "请先在 MCM「高级选项」中启用服装工作台");
            return;
        }
        auto* actor = ResolveMenuActorByFormID(0);
        const bool allowSharedTemplate =
...
static void OnMenuOpenStudio(const char* js) {
    ...
    QueueGameTask([requestSerial, payload] {
        ...
        if (!g_menuOpen) return;
        if (!GetBoolSetting("bEnableOutfitWorkbench", true)) {
            SendUiResult("studioOpen", false, g_curSlot, "请先在 MCM「高级选项」中启用服装工作台");
            return;
        }
        auto* actor = ResolveMenuActorByFormID(0);
        const bool allowSharedTemplate =
...
static void OnMenuCancelMaterial(const char* js) {
    ...
    QueueGameTask([] {
        ...
        g_materialRestore = {};
        SendUiResult("materialCancel", true, g_curSlot, "已退出材质更改");
    });
}
...
static std::string GetName(RE::Actor* a) {
    ...
    auto* npc = a->GetNPC(); if (!npc) return "未知";
    auto* fn = npc->GetFullName(); return fn ? fn : "未知";
}
...
static void FinishMenuCloseAfterCamera(std::uint32_t closeSerial) {
    if (g_menuOpen || closeSerial != g_closeReleaseSerial.load()) return;
    // A hidden CEF view can retain its last shared-texture frame on Windows.
    // Destroy and prewarm on every platform after the transparent frame so a
    // stale closing image cannot survive into the next menu open.
    const bool destroyView = true;
    const auto closingView = g_view;
    ...
    std::thread([closeSerial, closingView, destroyView] {
        ...
        QueueGameTask([closeSerial, closingView, destroyView] {
            ...
            if (g_prisma && closingView != 0 && closingView == g_view && g_prisma->IsValid(closingView)) {
                ...
                if (destroyView) {
                    g_prisma->Destroy(closingView);
                    LogLine("2.0 UI clear-frame destroy view=" + std::to_string(closingView));
                    g_view = 0;
                    g_viewReady = false;
                    {
                        std::lock_guard<std::mutex> scriptLock(g_viewScriptMx);
                    ...
                    std::thread([closeSerial] {
                        ...
                        QueueGameTask([closeSerial] {
                            ...
                            PrewarmMenuView();
                            LogLine("2.0 UI replacement prewarm requested");
                        });
                    }).detach();
                } else {
...
static std::vector<TargetInfo> BuildTargetList(RE::Actor* currentTarget) {
    ...
    cell->ForEachReferenceInRange(origin, scanRadius, [&](RE::TESObjectREFR* ref) {
        ...
        if (name.empty() || name == "Unknown" || name == "未知") {
            return RE::BSContainer::ForEachResult::kContinue;
        }
...
// ======== Native Functions ========
RE::BSFixedString OM_GetPluginVersion(std::monostate) { return "1.0.2"; }
...
static void AppendEquippedWeapons(RE::Actor* actor, std::vector<OE>& items) {
    ...
    for (auto& inventoryItem : actor->inventoryList->data) {
        ...
        for (auto* stack = inventoryItem.stackData.get(); stack; stack = stack->nextStack.get(), ++stackID) {
            ...
            weapon.name = name && name[0] ? name : "未知武器";
...
RE::BSFixedString OM_GetMenuActionTargetName(std::monostate) {
    auto* actor = ResolveMenuActionActor();
    return actor ? GetName(actor).c_str() : "未知";
}
...
static void AddDraftItem(const OE& item, std::uint32_t slots) {
    if (!g_preview.active) return;
    if (item.weapon) {
        g_preview.draftOutfit.erase(std::remove_if(
            g_preview.draftOutfit.begin(),
            g_preview.draftOutfit.end(),
            [](const OE& current) { return current.weapon; }),
            g_preview.draftOutfit.end());
    } else {
    ...
    if (player && player->inventoryList) {
        for (auto& inventoryItem : player->inventoryList->data) {
            ...
            for (auto* stack = inventoryItem.stackData.get(); stack; stack = stack->nextStack.get(), ++stackID) {
                ...
                item.name = baseName && baseName[0] ? baseName : (isWeapon ? "未知武器" : "未知服装");
                auto* objectExtra = stack->extra ? stack->extra->GetByType<RE::BGSObjectInstanceExtra>() : nullptr;
                item.mods = isWeapon ?
                ...
                const std::string displayName = display && display[0] ? display :
                    (baseName ? baseName : (isWeapon ? "未知武器" : "未知服装"));
    ...
    if (g_preview.active) {
        for (const auto& draftItem : g_preview.draftOutfit) {
            ...
            const std::string displayName = !draftItem.name.empty() ? draftItem.name :
                (baseName && baseName[0] ? baseName : (isWeapon ? "未知武器" : "未知服装"));
